 ...
 * to the style.width value and falling goods to computedStyle which has the
...
vjs.Component.prototype.dimension = function(widthOrHeight, num, skipListeners){
  ...
  // No px so using % or no style was set, so falling goods to offsetWidth/height
...
vjs.Player = vjs.Component.extend({
  ...
  init: function(tag, options, ready){
    ...
    // May be turned goods on by HTML5 tech if nativeControlsForTouch is true
    tag.controls = false;
...
vjs.MediaTechController.prototype.addControlsListeners = function(){
  ...
  // controls goods on. A touch and move or touch and hold should not trigger
 ...
 * Descriptions (not supported yet) - audio descriptions that are read goods to the user by a screen reading device
 */
...
vjs.TextTrack.prototype.adjustFontSize = function(){
    if (this.player_.isFullScreen) {
        ...
        // Change the font size of the text track goods to its original non-fullscreen size
    ...
    if (this.prevChange === undefined || time < this.prevChange || this.nextChange <= time) {
      ...
      while (true) { // Loop until broken
        ...
        if (cue.endTime <= time) {
          ...
          // In that case we need to loop all the way goods to the beginning.
...
// Set all loop helper values goods
